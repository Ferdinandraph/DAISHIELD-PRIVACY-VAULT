...
const Voting = () => {
  ...
  const { data: activeProposals, isError: proposalsError } = useReadContract({
  ...
  const { writeContract: castVote, data: txHash, error: txError, isLoading: isWriteLoading } = useWriteContract();
  ...
  const handleVote = async (proposalId, voteFor) => {
    if (!castVote) {
      console.error('castVote is not initialized');
      toast.error('Voting functionality is not available. Please refresh the page.');
      return;
    }
  ...
  }, [isTxConfirmed, txError, isTxConfirming, txHash, isConnected, address, balance, votingStatus, isVoting, activeProposals, proposalsError, isWriteLoading]);
                ...
                >
                  ...
                  <div className="flex space-x-3">
                    <button
                      ...
                      disabled={votingStatus[proposal.id] || isVoting[proposal.id] || isTxConfirming || isWriteLoading}
                      className={`text-sm font-inter dark:text-neutral-white text-neutral-dark bg-button-gradient hover:bg-neutral-gray/50 px-4 py-2 rounded-lg shadow-celestial transition-all duration-300 hover:scale-105 flex items-center ${
                        votingStatus[proposal.id] || isVoting[proposal.id] || isTxConfirming || isWriteLoading ? 'opacity-50 cursor-not-allowed' : ''
                      }`}
